...
const content = {
  header: {
    description:
      'React 19 adds better form actions, optimistic updates, and the use() API for reading async resources during render.',
    eyebrow: 'React 19',
    tags: ['React 19', 'Actions', 'Optimistic UI', 'use()'],
    title: 'React 19 Changes',
  },
  theory: {
    title: 'What changed in day-to-day React',
    summary:
      'React 19 reduces boilerplate around forms and async UI. Actions handle submission state, useOptimistic gives instant feedback, and use() reads promise or context values during render.',
    points: [
      'useActionState ties a form action to result state and pending status.',
      'useOptimistic renders an immediate temporary value while the real save is still happening.',
      'use() can read a promise in render when paired with Suspense, which is useful for async resources.',
    ],
    code: `const [state, formAction, isPending] = useActionState(saveProfile, initialState);
const [optimisticComments, addOptimisticComment] = useOptimistic(comments, addDraft);
const profile = use(profilePromise);`,
    whatToTry: [
      'Submit the profile form and watch pending state come from useActionState.',
      'Add an optimistic note and notice it appears before the simulated save finishes.',
    ],
  },
} as const;
...
export default function React19Page() {
  ...
  return (
    <LearningExamplePage
      codeFilePath="src/app/(learning)/react19/page.tsx"
